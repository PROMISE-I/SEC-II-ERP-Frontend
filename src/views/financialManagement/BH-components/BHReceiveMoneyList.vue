...
import {createReceiveMoney} from "@/network/financialManagement";
  ...
  methods: {
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          let id = this.receiveMoneyForm.id
          this.receiveMoneyForm.id += '-1'
    ...
    exportAsExcel() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        ...
        const filename = 'business-history-receive-money' + curTime

  ...
  methods: {
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          let id = this.saleReturnForm.id
          this.saleReturnForm.id += '-1'
    ...
    reverseCreate(id) {
      ...
      querySheetIdExist(config).then(_res => {
        ...
        for (let item of form.saleReturnsSheetContent) {
        ...
        form.id += '-0'
        console.log(form)
        createSaleReturn(form).then(_res => {
          if (_res.msg === 'Success') {
    ...
    exportAsExcel() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['id', '关联的销售单id', '操作员', '金额总计', '备注']
        const filterVal = ['id', 'saleSheetId', 'operator', 'finalAmount', 'remark']
        const list = this.list
        const data = this.formatJson(filterVal, list)
        const filename = 'business-history-sale-returns' + curTime
      ...
      for (let item of this.list) {
        for (let content of item.saleReturnsSheetContent) {
          let tmp = content
          tmp.outId = item.id
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['id', '商品id', '数量', '单价', '金额', '备注']
        const filterVal = ['outId', 'pid', 'quantity', 'unitPrice', 'totalPrice', 'remark']
        const list = contentList
        const data = this.formatJson(filterVal, list)
        const filename = 'business-history-sale-returns-content' + curTime
        excel.export_json_to_excel({
          header: tHeader,
          data,
          filename: filename,
